...
export async function POST() {
  ...
  if (resolved.length) {
    await env.DB.batch(resolved.map((item) => env.DB.prepare(
      "UPDATE conferences SET deadline = ?, abstract_deadline = ?, timezone = ?, source_name = ?, source_url = ?, deadline_status = ?, last_checked_at = ? WHERE id = ? AND manually_overridden = 0"
    ).bind(item.deadline, item.abstractDeadline, item.timezone, item.sourceName, item.sourceUrl, item.deadlineStatus, item.lastCheckedAt, item.id)));
  }
